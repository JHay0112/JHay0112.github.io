...
var skills = [
    [
        "fab fa-html5",
        "HTML5",
        "HTML5 (Hyper Text Markup Language 5) is a markup language to write websites. Jordan is proficient in HTML5 and has been using it since mid 2017.",
    ],
    [
        "fab fa-css3",
        "CSS3",
        "CSS3 (Cascading Style Sheet 3) is a language used to style HTML pages. Jordan has been using CSS3 since mid-2017.",
    ],
    [
        "fab fa-js",
        "JavaScript",
        "JavaScript is a programming language used to program the behaviour of websites. Jordan has been using JavaScript since early 2019.",
    ],
    [
        "fab fa-php",
        "PHP",
        "PHP is a programming language used to program the server-side behaviour of websites and to query databases. Jordan has been using PHP since early 2018.",
    ],
    [
        "fas fa-database",
        "MySQL",
        "MySQL is a language used to build and manage databases. Jordan has been using MySQL since early 2018.",
    ],
    [
        "fab fa-python",
        "Python",
        "Python is a general purpose programming language. Jordan has been using Python since mid 2018.",
    ],
    [
        "fab fa-git-alt",
        "Git",
        "Git is a version control system used to manage code. Jordan has been using Git since late 2017.",
    ],
    [
        "fab fa-github",
        "Github",
        "Github is an online host for version control with Git. Jordan has been using Github since late 2017.",
    ],
    [
        "fas fa-microchip",
        "Arduino/Robotics",
        "Arduino is a microcontroller and language used for hobby electronics including robotics and automation. Jordan has had experience with Arduino since late 2018.",
    ],
    [
        "fas fa-cube",
        "3D Printing/CAD",
        "3D Printing is the use of 3D printers to create 3D objects. Computer Aided Design is a method for developing designs and models with computers. Jordan has been using CAD for developing objects to 3D print and has learnt how to maintain a 3D printer since late 2018.",
    ],
    [
        "fas fa-palette",
        "GIMP",
        "GIMP (GNU Image Manipulation Program) is used to manipulate images and is what Jordan uses to create graphics and edit photos. Jordan has been using GIMP since early 2018.",
    ],
    [
        "fab fa-ubuntu",
        "Linux",
        "Jordan has been using Ubuntu and Ubuntu derivative operating systems for both personal and server use since early 2018.<br /><br />Please note: 'Server' above refers to an old personal computer Jordan installed Ubuntu Server 16.04 on and used to host various web projects and a Discord bot (written in Python).",
    ],
    [
        "fas fa-camera-retro",
        "Photography",
        "Jordan has been using a DSLR camera (Canon EOS 350D) since mid 2018 and a compact camera (Olympus TG-630) before that. He was also the photographer for his school whanau/house for the years of 2018 and 2019.",
    ],
    [
        "fas fa-plug",
        "Hobby Electronics",
        "Jordan has experience in hobby electronics including soldering, circuit design/analysis, PCB design, using a multimeter, and oscilliscope.",
    ],
    [
        "fas fa-network-wired",
        "Networking",
        "Jordan has experience with setting up and managing networks, most of this learnt while putting together a home-wide ethernet LAN, and while experimenting with an old computer he used as a server for various web projects.",
    ],
];
...
async function exitLoadingScreen() {
    ...
    loadingSpinner.style.animation =
        "spin 1s infinite linear, fade-out 2s forwards"; // Fade out while still spinning
    loadingScreen.style.animation =
        "slide-out-bottom 1.3s 1.5s forwards ease-out";
    await sleep(1200);
...
function load404Image() {
    ...
    imgElement.src =
        "/img/404/" + images[Math.floor(Math.random() * images.length)];
}
...
async function toggleDarkMode(scheme = 0) {
    ...
    if ((darkMode == false && scheme != 2) || scheme == 1) {
        // Fires only if the desired scheme isn't white mode and it isn't currently dark mode,
        // or the desired scheme is dark mode
        textColour = "white";
        backgroundColour = "#252525";
        imageFilter = "brightness(0) invert(1)";
        darkModeIcon = "fas fa-sun";
        darkMode = true;
    } else if (darkMode == true && scheme != 1) {
        // Fires if is dark mode and the desired scheme isn't dark mode
        darkMode = false;
    }

    Object.keys(all).forEach(
        (e) => (all[e].style.transition = "0.3s ease all")
    ); // Set every element transition
    ...
    if (
        location.href == "https://jordanhay.tk/" ||
        location.href.includes("index") ||
        location.href.includes("jhay0112.github.io/index.html") ||
        location.href == "https://jordanhay/"
    ) {
...
function checkDarkMode() {
    if (window.matchMedia) {
        // Check that the client supports the feature
        var listener = function ({ matches, media }) {
            if (!matches) {
                return;
                // Exits the program if the opposite to desired listener is fired
            }
            if (!window.localStorage.getItem("dark-mode")) {
                // Makes sure a user override isn't in place
                if (
                    media === "(prefers-color-scheme: dark)" &&
                    darkMode == false
                ) {
                    toggleDarkMode(1);
                    // Fires if the client changed to dark mode and it is light mode
                } else if (
                    media === "(prefers-color-scheme: light)" &&
                    darkMode == true
                ) {
                    toggleDarkMode(2);
                    // Fires if the client changed to light mode and it is dark mode
                }
            }
        };
        ...
        // Adds listeners, one for each color scheme
    }
    if (window.localStorage.getItem("dark-mode") == "true") {
        toggleDarkMode();
        return;
    }
    // Checks if the localstorage setting exists, then sets the color scheme and exits

    listener(window.matchMedia("(prefers-color-scheme: dark)"));
    // Checks if the client prefers dark mode if there isn't a localstorage override
    // Function to check the color scheme, add a listener for color scheme changes,
    // and check the color scheme on load
}
...
window.onscroll = function () {
    scrollFunction();
}; // Run the scroll function anytime the user scrolls
